    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="font-heading text-3xl font-bold text-white">AI Flashcards</h1>
            <p className="text-[#a0a0a0]">Master definitions, formulas, and concepts from specific study materials.</p>
          </div>
          <button
            ...
            <div className="bg-card border border-border rounded-2xl p-6 max-w-md w-full space-y-6 shadow-2xl relative">
              <div className="flex justify-between items-center border-b border-border pb-4">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  <FileText className="w-5 h-5 text-primary" /> Flashcard Deck Configuration
                </h2>
        ...
        {loading ? (
          <div className="p-12 text-center text-[#a0a0a0] flex items-center justify-center gap-2">
            <Loader2 className="w-5 h-5 animate-spin text-primary" /> Loading flashcard deck...
          </div>
        ) : flashcards.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-border rounded-2xl bg-input/20">
            ...
            <p className="text-sm text-[#a0a0a0] mb-4">Click below to extract active recall cards from your notes.</p>
            <button
              onClick={() => setShowModal(true)}
              ...
              return (
                <div
                  ...
                  className={`cursor-pointer min-h-[220px] rounded-2xl p-6 border transition-all duration-300 flex flex-col justify-between ${
                    isFlipped
                      ? 'bg-gradient-to-br from-primary/20 to-primary/5 border-primary/50 shadow-xl bg-card'
                ...
                >
                  <div>
                    ...
                    <p className={`text-sm leading-relaxed ${isFlipped ? 'text-primary/90' : 'text-[#e0e0e0]'}`}>
                      {isFlipped ? card.answer : card.question}
                    </p>
                  </div>

                  <div className="text-xs text-[#a0a0a0] pt-4 border-t border-border flex justify-between items-center">
                    <span>Type: {card.type || 'concept'}</span>
                    <span className="text-primary font-medium">{isFlipped ? 'Click to show question' : 'Click to flip card'}</span>
                  </div>
                </div>
              );
            })}
